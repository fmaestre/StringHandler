...
namespace TerminalTools
{
    public class TermServicesManager
    {
                ...
                if (retval != 0)
                {
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        if (si.State == WTS_CONNECTSTATE_CLASS.Active)
                           ret.Add(new TerminalSessionData(si.SessionID, si.State, si.pWinStationName, GetSessionNameInfo(ServerName, si.SessionID))); 
                    }
        ...
        public static TerminalSessionInfo GetSessionNameInfo(string ServerName, int SessionId)
        {
            IntPtr server = IntPtr.Zero;
            server = OpenServer(ServerName);
            System.IntPtr buffer = IntPtr.Zero;
    ...
    public class TerminalSessionData
    {
        ...
        public TerminalSessionInfo Info;

        public TerminalSessionData(int sessionId, TermServicesManager.WTS_CONNECTSTATE_CLASS connState, string stationName, TerminalSessionInfo info = null)
        {
            SessionId = sessionId;
        ...
        {
            return String.Format("{0} ---- {1} ---- {2} ---- {3} ---- {4}", SessionId, ConnectionState, StationName, Info.UserName, Info.ClientName);
        }
    }
